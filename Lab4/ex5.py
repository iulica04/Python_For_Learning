...
def search_in_files(file, to_search):
    try:
...
def find_files_with_content(target, to_search):
    try:
        if not os.path.exists(target):
            raise Exception("The path doesn't exists.")

        if os.path.isfile(target):
            if search_in_files(target, to_search):
                return list(target)
        elif os.path.isdir(target):
            files_with_content = list()
            for root, dirs, files in os.walk(target):
                for file in files:
                    if search_in_files(os.path.join(root, file), to_search):
...
if __name__ == "__main__":
    print(find_files_with_content("D:\\Facultate 3.1\\Python\\Lab4", "def"))